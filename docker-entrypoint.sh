...
PUBLIC_NOVNC_PROXY_ENABLED=false
NOVNC_PUBLIC_PATH=""
if [ "$PINCHTAB_ENABLED_JSON" = "true" ] && [ "$PINCHTAB_NOVNC_ENABLED_JSON" = "true" ] && [ -n "$PINCHTAB_NOVNC_PUBLIC_PATH" ]; then
  ...
  PUBLIC_NOVNC_PROXY_ENABLED=true
  NULLCLAW_GATEWAY_PORT="$NULLCLAW_GATEWAY_INTERNAL_PORT"
fi

NOVNC_PUBLIC_URL=""
if [ -n "$RAILWAY_PUBLIC_DOMAIN" ] && [ "$PUBLIC_NOVNC_PROXY_ENABLED" = "true" ]; then
...
EOF_CHANNELS
  ...
  AGENT_PINCHTAB_API_URL="http://127.0.0.1:${PINCHTAB_PORT}"
...
- PinchTab API (internal): \`${AGENT_PINCHTAB_API_URL}\`
...
if [ "$PUBLIC_NOVNC_PROXY_ENABLED" = "true" ]; then
...
EOF_CADDY
  echo "Starting caddy public proxy on 0.0.0.0:${PUBLIC_PORT} (noVNC path=${NOVNC_PUBLIC_PATH})"
